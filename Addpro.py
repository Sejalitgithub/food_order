...
class addpro:
 ...
 # Quantity
        ...
        btnupdate = Button(btn_frame, text="Update",command=self.update, font=("arial", 11, "bold"), bg="black", fg="gold", width=11)
        btnupdate.grid(row=0, column=1, padx=1)

        btndelete = Button(btn_frame,command=self.mDelete, text="Delete", font=("arial", 11, "bold"), bg="black", fg="gold", width=11)
    ...
    # def add_Gdata(self):
    #     if self.var_category.get()=="Gujrati":
    #     #     messagebox.showerror("Error","All fields are required",parent=self.root)
    #     # else:
    #     #      try:
    #              conn=mysql.connector.connect(host="localhost",user="root",password="sejal@0712",database="food_order", auth_plugin="mysql_native_password")
    #              my_cursor=conn.cursor()
    #              my_cursor.execute("insert into gujrati values(%s,%s,%s,%s,%s,%s)",(self.var_ref.get(),self.var_category.get(),self.var_pname.get(),self.var_price.get(),self.var_quantity.get(),self.var_details.get()))
    #
    #              conn.commit()
    #              self.fetch_data()
    #              conn.close()
