...
class Bfood:
    def __init__(self,root):
        ...
        lb1_title = Label(self.root, text="GUJRATI FOOD", font=("times new roman", 18, "bold"), bg="black",
        ...
        #### LABELE FRAME #####
        labelframeleft = LabelFrame(self.root, bd=2, relief=RIDGE, text=" Breakfast items",font=("arial", 12, "bold"),padx=2)
    ...
    # Product ref
        ...
        combo_category["value"] = ("Breakfast")
 ...
 # Quantity
  ...
  # fetch button is not workinb
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",user="root",password="sejal@0712",database="food_order",auth_plugin="mysql_native_password")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from breakfast")
...
#==========================  Update BUtton ================================
    ...
    def search(self):
        conn = mysql.connector.connect(host="localhost", user="root", password="sejal@0712", database="food_order", auth_plugin="mysql_native_password")
        my_cursor = conn.cursor()
        my_cursor.execute("select * from breakfast where " + str(self.search_var.get()) + " LIKE '%" + str(self.txt_search.get()) + "%'")
        rows = my_cursor.fetchall()
        if len(rows) != 0:
            self.food_Details_table.delete(*self.food_Details_table.get_children())
            for i in rows:
